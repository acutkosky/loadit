...
class RandomAccess:
    def __init__(
        ...
        self.shards = ShardedDataset(shard_dir, max_shard_length)

        self.shard_access_times = self.shards.root_dir / "access_times.json"
        self.access_times_lock = FileLock(
            self.shards.root_dir / "access_times.lock"
        )

        self.cache = SimpleCache(max_cache_size)
        executor = ThreadPoolExecutor(max_workers=max_workers)
        self.writer_pool = WriterPool(
            self.shards, self.cache, iterator=it, executor=executor, iterator_generator=iterator_generator, num_workers=1
        )
